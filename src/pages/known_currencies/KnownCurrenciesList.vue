<template>
  <v-flex xs12 sm12>
    <v-container>
      <v-subheader class="headline">
        Known Currencies
        <v-spacer/>
        <v-btn @click="addKnownCurrency">Add Currency</v-btn>
        <v-btn @click="onRefresh">Refresh</v-btn>
      </v-subheader>
      <div v-if="errorMessages.length > 0">
        <v-subheader v-for="error in errorMessages" :key="error.error_code" class="error">{{ error.error_message }}</v-subheader>
      </div>
      <v-data-table
        :headers="headers"
        :items="items"
        :loading="knownCurrenciesListStatus.loading"
        :pagination.sync="pagination"
        hide-actions
        class="elevation-1">
        <template slot="items" slot-scope="props">
          <tr>
            <td @click="editKnownCurrency(props.item.id)">{{ props.item.name }} <br> {{ props.item.issuer_public_key }}</td>
            <td>{{ props.item.asset_code }}</td>
            <td>{{ props.item.short_description }}</td>
            <!-- <td>{{ props.item.long_description }}</td> -->
            <td>
              <v-btn :disabled="props.item.order_index <= 1" flat icon @click="changeOrder(props.item.id, -1)">
                <v-icon>keyboard_arrow_up</v-icon>
              </v-btn>
              {{ props.item.order_index }}
              <v-btn :disabled="props.item.order_index >= items.length" flat icon @click="changeOrder(props.item.id, 1)">
                <v-icon>keyboard_arrow_down</v-icon>
              </v-btn>
            </td>
          </tr>
        </template>
      </v-data-table>
    </v-container>
  </v-flex>
</template>

<script>
import { mapActions, mapGetters } from 'vuex';

import { Urls } from '@/router/urls';

export default {
  name: 'KnownCurrenciesList',
  data () {
    return {
      headers: [
        { text: 'Name / Public key', value: 'nameAndPublicKey' },
        { text: 'Asset Code', value: 'asset_code' },
        { text: 'Short Description', value: 'short_description' },
        // { text: 'Long Description', value: 'long_description' },
        { text: 'Order Index', value: 'order_index' },
      ],
      pagination: {'sortBy': 'order_index', 'ascending': true, 'rowsPerPage': -1},
      errorMessages: [],
    };
  },
  computed: {
    ...mapGetters(['knownCurrenciesListStatus', 'knownCurrenciesList']),
    items () {
      if (!this.knownCurrenciesList) return [];
      return this.knownCurrenciesList;
    },
  },
  created () {
    this.getKnownCurrenciesList();
  },
  methods: {
    ...mapActions(['getKnownCurrenciesList']),
    addKnownCurrency (id) {
      this.$router.push({ path: `known_currencies/add` });
    },
    editKnownCurrency (id) {
      this.$router.push({ path: `known_currencies/${id}` });
    },
    async changeOrder (id, orderModifier) {
      this.loading = true;
      this.errorMessages = [];
      try {
        await this.$http({
          url: Urls.KnownCurrencies.ChangeCurrencyOrder,
          method: 'POST',
          data: {
            id: parseInt(id, 10),
            order_modifier: parseInt(orderModifier, 10),
          }
        });
        await this.getKnownCurrenciesList();
      } catch (err) {
        this.errorMessages = err.response.data;
        this.loading = false;
      }
      this.loading = false;
    },
    onRefresh () {
      this.getKnownCurrenciesList();
    }
  }
};
</script>

<style lang="scss" scoped>
</style>
